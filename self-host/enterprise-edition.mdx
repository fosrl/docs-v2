...
<Card title="Get a License" icon="server" href="https://pangolin.net/pricing#Self-Hosted">
...
<Info>
  Looking for the exact feature differences between Community Edition and Enterprise Edition? See the [self-hosted pricing comparison](https://pangolin.net/pricing#Self-Hosted) for the current feature matrix. You will also see Enterprise-only features called out throughout the docs when they apply.
</Info>
...
To see pricing tiers for the self-hosted Enterprise Edition for businesses, [click here](https://pangolin.net/pricing#Self-Hosted). For more on how to purchase a key, see [Purchase a license key](/self-host/purchase-license-key).
...
<Step title="Create an organization">
  After signing up you will be prompted to create an organization. This is required to apply for a license key. You can use the default organization name or create a custom one.
</Step>
...
<Accordion title="Can I get a trial license?">
  ...
  Businesses that don't qualify for free personal licensing can reach out to support@pangolin.net for a trial license key to test Enterprise features before purchasing.
...
<AccordionGroup>
<Accordion title="Which features are in Enterprise Edition but not Community Edition?">
  Enterprise Edition includes everything in Community Edition, then unlocks additional paid features for self-hosted deployments.
  
  For the current list of features available in each self-hosted edition, use the [self-hosted pricing comparison](https://pangolin.net/pricing#Self-Hosted). The docs intentionally link to the pricing page instead of recreating the comparison table here, so there is only one place to keep feature availability up to date.
</Accordion>
