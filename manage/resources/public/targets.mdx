...
In the public resource **Proxy** tab, Pangolin also supports per-resource proxy behavior controls beyond target routing.

### TLS Server Name

Sets the TLS Server Name (SNI) used when connecting to backend targets.  
Leave empty to use the default behavior.

### Enable Sticky Sessions

When enabled, Pangolin keeps a client session pinned to the same backend target for the duration of that session.

### Custom Host Header

Overrides the `Host` header forwarded to the backend target.  
Leave empty to forward the default host header.

### Custom Headers

Adds static headers to proxied requests.
...
Use **Save Proxy Settings** in the UI after modifying these values.
